...
<div id="wrapper">
 ...
 <h6 style="text-transform:uppercase;"><b>CEDAR CLINK HARDWARE

</b></h6>
             <h7><b> <?php echo $warehousename->name; ?></b><br></h7>
            <?php
			$data1 = 'RECEIPT NO: '.$inv->id.''. PHP_EOL;
						$this->load->helper('file');
                //echo "PIN # - P051615194Q<br>";
                echo "TELEPHONE 0724230729/0707580060<br>";
          
               // echo "P.O Box 103605-00101 Nairobi.<br>";
                echo "Email: sales@cedarclink.com<br>";
                echo "PAYBILL NO:4038019<br>";
           ...
           // echo "<p>" . $biller->address . " " . $biller->city . " " . $biller->postal_code . " " . $biller->state . " " . $biller->country .
            ...
            <?php }
            echo "<p>" . lang("Receipt_no") . ": " . substr($inv->reference_no, 17) . "<br>";
           ...
           // echo "Chef" . ": " . $inv->chef . "</br>";
            ...
            <table class="table table-striped table-condensed" border="1" width="100%">
                <tbody>
                    <tr>
                        <th  colspan="2">Item</th> <th> Qty * Price</th>
                ...
                foreach ($rows as $row) {
                    ...
                    echo '<tr><td colspan="2" style="font-size:12px"> &nbsp;&nbsp;' . product_name($row->product_name) . ($row->variant ? ' (' . $row->variant . ')' : '') . '<span class="pull-right"></span></td>';
                    $data7 = '#' . $r . ':  ' . product_name($row->product_name) . ($row->variant ? ' (' . $row->variant . ')' : '') .PHP_EOL;
					echo '<td style="font-size:12px">' . number_format($row->quantity) . ' x ';
                    ...
                    echo $this->sma->formatMoney($row->net_unit_price + ($row->item_tax / $row->quantity)) . '</td><td class="text-right" style="font-size:12px">' . $this->sma->formatMoney($row->subtotal) . '</td></tr>';
                ...
                <tr style="display:none">
                    <th style="font-size:12px"><?= lang("total"); ?></th>
                    <th style="font-size:12px" class="text-right"><?= $this->sma->formatMoney($inv->total + $inv->product_tax); ?></th>
					<?php 
                ...
                ?>
                    ...
                    <tr style="display:none">
                        <th style="font-size:12px"><?= lang("grand_total"); ?></th>
                ...
                if ($inv->paid < $inv->grand_total) { ?>
                 <!--   <tr>
                    ...
                    <tr>
                        <th style="font-size:12px" colspan="2"><?= lang("due_amount"); ?></th> <th></th>
                ...
                <?php }
                 ...
                 ?>
                 <tr>
                        <th  colspan="2"><?= "<b >Total </b>"; ?></th> <th></th>
                        <th class="text-right"><?= $this->sma->formatMoney($inv->grand_total); ?></th>
                  </tr>
            ...
            if ($payments) {
                ...
                foreach ($payments as $payment) {
                    ...
                    if ($payment->paid_by == 'cash' && $payment->pos_paid) {
                       ...
                       echo '<td>' . lang("amount") . ': ' . $this->sma->formatMoney($payment->pos_paid? ($payment->amount + $payment->pos_balance) : $payment->pos_paid) . ($payment->return_id ? ' (' . lang('returned') . ')' : '') . '</td>';
	...
	// write_file('C:/etr.txt',$data1.$data2.$data3.$data4.$data5.$data6.$data7.$data8.$data9.$data10.$data11.$data12.$data13);
            ...
            <div class="well well-sm">
                <?='<p>You were served by:'.$soldby->first_name.'&nbsp;'.$soldby->last_name.'</p>';?>
