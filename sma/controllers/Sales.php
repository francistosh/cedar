...
//         
    ...
    function getSales($warehouse_id = NULL)
    {
        ...
        if (!$this->Owner && !$warehouse_id) {
         ...
         $complete_payment = anchor('sales/complete_payment/$1', '<i class="fa fa-money"></i> ' . " Complete Payment", 'data-toggle="modal" data-target="#myModal"');
      ...
      //  $add_delivery_link = anchor('delivery/add/$1', '<i class="fa fa-edit"></i> ' . lang('add_delivery'), 'class="sledit"');
    ...
    function getReturns($warehouse_id = NULL)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select($this->db->dbprefix('return_sales') . ".date as date, " . $this->db->dbprefix('return_sales') . ".reference_no as ref, " . $this->db->dbprefix('sales') . ".reference_no as sal_ref, " . $this->db->dbprefix('return_sales') . ".salesperson, " . $this->db->dbprefix('return_sales') . ".customer, " . $this->db->dbprefix('return_sales') . ".surcharge, " . $this->db->dbprefix('return_sales') . ".grand_total, " . $this->db->dbprefix('return_sales') . ".id as id")
    ...
    function view($id = NULL)
    {
        ...
        $this->data['biller'] = $this->site->getCompanyByID($inv->biller_id);
        $this->data['bank'] = $this->site->getBankByID($inv->bank_id);
        $this->data['created_by'] = $this->site->getUser($inv->created_by);
        $this->data['updated_by'] = $inv->updated_by ? $this->site->getUser($inv->updated_by) : NULL;
        $this->data['warehouse'] = $this->site->getWarehouseByID($inv->warehouse_id);
        $this->data['inv'] = $inv;
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateSale($id, $data, $products)) {
            ...
            if ($this->data['inv']->date <= date('Y-m-d', strtotime('-6 months'))) {
                $this->session->set_flashdata('error', lang("sale_x_edited_older_than_6_months"));
                redirect($_SERVER["HTTP_REFERER"]);
            }
    ...
    function delete($id = NULL)
    {
       ...
       // die($id);
        $sale=$this->sales_model->getInvoiceByID($id);
       // die(print_r($this->session->userdata("username")));
       // $smsdetails=array('phone'=>"+254729790941",'message'=>"Sale Ref ".$sale->reference_no." created on ".$sale->date." of KES".$sale->grand_total." and Paid KES ".$sale->paid." deleted by ".$this->session->userdata("username"));
                         //  $this->SendCustomerSms($smsdetails); 
    ...
    private function SendCustomerSms($smsdetails){
        $settings=array("afist_username"=>"posii","afist_pwd"=>"f0c1d53b27bc1be5d176dd26ed2c2954e58f937063be1d946d90e4b057d276fa");
// Specify the numbers that you want to send to in a comma-separated list
// Please ensure you include the country code (+254 for Kenya in this case)
$recipients = "+254706160116";//$smsdetails['phone'];
            ...
            $ccref=$this->input->post('reference');
               ...
               // 'cc_cvv2' =>"",
                ...
                if ($this->input->post('form_action') == 'export_excel' || 
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('D1', lang('sold_by'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $sale->biller);
                         $userdetails=$this->site->getUser($sale->created_by);
                          $this->excel->getActiveSheet()->SetCellValue('D' . $row,$userdetails->first_name." ".$userdetails->last_name);
...
//    }
    ...
    <ul class="dropdown-menu pull-right" role="menu">
        ...
        $this->datatables
            ->select("sma_deliveries.id as id, date, do_reference_no, sale_reference_no, customer, address")
            ->from('deliveries')
    ...
    function add_payment($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $payment = array(
                ...
                'paid_by' => $this->input->post('paid_by'),
                'cheque_no' => $this->input->post('cheque_no'),
                'creditnote_rmks'=> $this->input->post('crdnt_det'),
