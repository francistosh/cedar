...
//        }
    ...
    function getProducts($warehouse_id = NULL)
    {
        ...
        $this->load->library('datatables');
         if ($warehouse_id) {
            $this->datatables
                ->select($this->db->dbprefix('warehouses_products') . ".product_id as productid, " . $this->db->dbprefix('products') . ".image as image, " . $this->db->dbprefix('products') . ".code as code, " . $this->db->dbprefix('products') . ".name as name, " . $this->db->dbprefix('categories') . ".name as cname, cost as cost, price as price, price as price, cf1 as Best_Price, " . $this->db->dbprefix('warehouses_products') . ".quantity as quantity, unit, " . $this->db->dbprefix('warehouses_products') . ".rack as rack, alert_quantity", FALSE)
            ...
            $this->datatables
                ->select($this->db->dbprefix('products') . ".id as productid, " . $this->db->dbprefix('products') . ".image as image, " . $this->db->dbprefix('products') . ".code as code, " . $this->db->dbprefix('products') . ".name as name, " . $this->db->dbprefix('categories') . ".name as cname, cost as cost, price as price, cf1 as Best_Price,  " . $this->db->dbprefix('warehouses_products') . ".quantity as quantity, unit, NULL as rack, alert_quantity", FALSE)
                ->from('products')
                ->join('categories', 'products.category_id=categories.id', 'left')
...
//    }
    ...
    function update_price()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                $keys = array('code','cost', 'price','cf1');
    ...
    function getadjustments($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ->select($this->db->dbprefix('adjustments') . ".id as did, " . $this->db->dbprefix('adjustments') . ".product_id as productid, " . $this->db->dbprefix('adjustments') . ".date as date, " . $this->db->dbprefix('products') . ".image as image, " . $this->db->dbprefix('products') . ".code as code, " . $this->db->dbprefix('products') . ".name as pname, " . $this->db->dbprefix('product_variants') . ".name as vname, " . $this->db->dbprefix('adjustments') . ".quantity as quantity, ".$this->db->dbprefix('adjustments') . ".type, ".$this->db->dbprefix('users') . ".username as created_by," . $this->db->dbprefix('warehouses') . ".name as wh");
            ...
            $this->db->join('users', 'users.id=adjustments.created_by', 'left');
            ...
            $this->datatables->join('users', 'users.id=adjustments.created_by', 'left');
