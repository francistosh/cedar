...
class Delivery extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        $this->lang->load('delivery', $this->Settings->language);
    ...
    function getDeliveries($warehouse_id = NULL)
    {
        ...
        $detail_link = anchor('delivery/view/$1', '<i class="fa fa-file-text-o"></i> ' . lang('delivery_details'));
        $email_link = anchor('delivery/email/$1', '<i class="fa fa-envelope"></i> ' . lang('email_delivery'), 'data-toggle="modal" data-target="#myModal"');
        $edit_link = anchor('delivery/edit/$1', '<i class="fa fa-edit"></i> ' . lang('edit_delivery'));
        ...
        $pdf_link = anchor('delivery/pdf/$1', '<i class="fa fa-file-pdf-o"></i> ' . lang('download_pdf'));
        $delete_link = "<a href='#' class='po' title='<b>" . $this->lang->line("delete_delivery") . "</b>' data-content=\"<p>"
            . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('delivery/delete/$1') . "'>"
    ...
    function modal_view($delivery_id = NULL)
    {
        ...
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $inv = $this->delivery_model->getDeliveryByID($delivery_id);
        //$this->sma->view_rights($inv->created_by, TRUE);
        //print_r($this->delivery_model->getAllDeliveryItems($delivery_id));
        //die();
        $this->data['rows'] = $this->delivery_model->getAllDeliveryItems($delivery_id);
        ...
        $this->load->view($this->theme.'deliveries/modal_view', $this->data);

    }

    function view($delivery_id = NULL)
    {
        $this->sma->checkPermissions('index');
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('delivery'), 'page' => lang('deliveries')), array('link' => '#', 'page' => lang('view')));
        $meta = array('page_title' => lang('view_delivery_details'), 'bc' => $bc);
        $this->page_construct('deliveries/view', $meta, $this->data);

    }

    function pdf($delivery_id = NULL, $view = NULL, $save_bufffer = NULL)
    {
        $this->sma->checkPermissions();
        ...
        $this->data['warehouse'] = $this->site->getWarehouseByID($inv->warehouse_id);
        $this->data['inv'] = $inv;
        $name = $this->lang->line("delivery") . "_" . str_replace('/', '_', $inv->reference_no) . ".pdf";
        $html = $this->load->view($this->theme . 'deliveries/pdf', $this->data, TRUE);
        if ($view) {
            $this->load->view($this->theme . 'deliveries/pdf', $this->data);
        } elseif ($save_bufffer) {
    ...
    function email($delivery_id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
           // $this->sma->view_rights($inv->created_by);
            ...
            $message = $this->parser->parse_string($msg, $parse_data);
			 ...
			 //if an extra attchment exists
            if(!empty($inv->attachment)){
           
           // $attachment = array($this->pdf($delivery_id, NULL, 'S'),'files/Terms & Conditions.pdf');
           $attachment = "";
           }
            else{
             $attachment = $this->pdf($delivery_id, NULL, 'S');    
           }
        ...
        if ($this->form_validation->run() == true && $this->sma->send_email($to, $subject, $message, NULL, NULL, $attachment, $cc, $bcc)) {
            $this->db->update('delivery', array('status' => 'sent'), array('id' => $delivery_id));
            delete_files($attachment);
            //$this->delivery_model->updateQuote
            ...
            if (file_exists('./themes/' . $this->theme . '/views/email_templates/delivery.html')) {
                $quote_temp = file_get_contents('themes/' . $this->theme . '/views/email_templates/delivery.html');
            } else {
                $quote_temp = file_get_contents('./themes/default/views/email_templates/delivery.html');
            }
            $this->session->set_flashdata('message', $this->lang->line("email_sent"));
            $this->data['subject'] = array('name' => 'subject',
                'id' => 'subject',
                'type' => 'text',
                'value' => $this->form_validation->set_value('subject', 'Delivery (' . $inv->reference_no . ') from ' . $this->Settings->site_name),
            );
            ...
            $this->data['id'] = $delivery_id;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'deliveries/email', $this->data);
    ...
    function add()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $unit_cost = "unit_cost";
            $tax_rate = "tax_rate";
            $reference = $this->input->post('reference_no') ? $this->input->post('reference_no') : $this->site->getReference('do');
           ...
           // }
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                //$option_details = $this->delivery_model->getProductOptionByID($item_option);
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    $product_details = $item_type != 'manual' ? $this->delivery_model->getProductByCode($item_code) : NULL;
        ...
        if ($this->form_validation->run() == true && $this->delivery_model->addDelivery($data, $products)) {
          ...
          //  $this->data['warehouses'] = $this->Owner ? $this->site->getAllWarehouses() : NULL;
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('delivery'), 'page' => lang('deliveries')), array('link' => '#', 'page' => lang('add_delivery')));
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
           ...
           // }
            ...
            $data = array('created_on'=>$createdon,
                'date' => $date,
                'reference_no' => $reference,
                'customer_id' => $customer_id,
                'customer' => $customer,
        ...
        if ($this->form_validation->run() == true && $this->delivery_model->updateDelivery($id, $data, $products)) {
            $this->session->set_userdata('remove_quls', 1);
            $this->session->set_flashdata('message', $this->lang->line("delivery_added"));
            redirect('delivery');
        } else {
            ...
            $this->data['inv'] = $this->delivery_model->getDeliveryByID($id);
            $inv_items = $this->delivery_model->getAllDeliveryItems($id);
            ...
            foreach ($inv_items as $item) {
                ...
                $row->quantity = 0;
                $pis = $this->delivery_model->getPurchasedItems($item->product_id, $item->warehouse_id, $item->option_id);
                if($pis){
                    foreach ($pis as $pi) {
                ...
                $options = $this->delivery_model->getProductOptions($row->id, $item->warehouse_id);

                if ($options) {
                    $option_quantity = 0;
                    foreach ($options as $option) {
                        $pis = $this->delivery_model->getPurchasedItems($row->id, $item->warehouse_id, $item->option_id);
                        if($pis){
                            foreach ($pis as $pi) {
           ...
           // $this->data['billers'] = $this->Owner ? $this->site->getAllCompanies('biller') : NULL;
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('delivery'), 'page' => lang('deliveries')), array('link' => '#', 'page' => lang('edit_delivery')));
            $meta = array('page_title' => lang('edit_delivery'), 'bc' => $bc);
            $this->page_construct('deliveries/edit', $meta, $this->data);
        }
    }
    
    function add_itemdelivery($id = NULL)
    {
        $this->sma->checkPermissions();
        ...
        if ($this->form_validation->run() == true && $this->delivery_model->updateDelivery($id, $data, $products)) {
           ...
           // $this->data['billers'] = $this->Owner ? $this->site->getAllCompanies('biller') : NULL;
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('delivery'), 'page' => lang('deliveries')), array('link' => '#', 'page' => lang('edit_delivery')));
            $meta = array('page_title' => lang('edit_delivery'), 'bc' => $bc);
            $this->page_construct('deliveries/add_itemdelivery', $meta, $this->data);
        }
    }
    ...
    function delete($id = NULL)
    {
        ...
        if ($this->delivery_model->deleteDelivery($id)) {
            if($this->input->is_ajax_request()) {
                echo lang("delivery_deleted"); die();
            }
    ...
    function suggestions()
    {
        ...
        $rows = $this->delivery_model->getProductNames($sr, $warehouse_id);
        if ($rows) {
            foreach ($rows as $row) {
                $option = FALSE;
                $row->quantity = 0;
                ...
                $row->qty = 1;
                $row->discount = '0';
                $options = $this->delivery_model->getProductOptions($row->id, $warehouse_id);
                if ($options) {
                    $opt = $options[0];
                    if (!$option) {
                        $option = $opt->id;
                ...
                $row->option = $option;
                $pis = $this->delivery_model->getPurchasedItems($row->id, $warehouse_id, $row->option);
                if($pis){
                    foreach ($pis as $pi) {
                        $row->quantity += $pi->quantity_balance;
                    }
                ...
                if ($row->tax_rate) {
                    $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
                    if ($row->type == 'combo') {
                        $combo_items = $this->delivery_model->getProductComboItems($row->id, $warehouse_id);
                    }
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    $this->session->set_flashdata('message', $this->lang->line("deliverys_deleted"));
                    redirect($_SERVER["HTTP_REFERER"]);
                }
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $row = 2;
                    foreach ($_POST['val'] as $id) {
                        $qu = $this->delivery_model->getDeliveryByID($id);
                ...
                $this->session->set_flashdata('error', $this->lang->line("no_delivery_selected"));
                redirect($_SERVER["HTTP_REFERER"]);
            }
        } else {
