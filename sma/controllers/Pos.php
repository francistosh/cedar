...
//                ->group_by('sales.id');
        ...
        if (!$this->Customer && !$this->Supplier && !$this->Owner && !$this->Admin) {
          $this->datatables->where('DATE_FORMAT('.$this->db->dbprefix('sales').'.date,"%Y-%m-%d") = CURDATE()');
            ...
            if ($sid) {
                ...
                foreach ($inv_items as $item) {
                    ...
                    if ($row->tax_rate) {
                        ...
                        $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'tax_rate' => $tax_rate, 'options' => $options,'cf1'=>$row->cf1);
                    } else {
                        $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'tax_rate' => false, 'options' => $options,'cf1'=>$row->cf1);
                    }
                    $c++;
                }
       ...
       // $this->sma->checkPermissions('payments', true);
        ...
        if ($this->form_validation->run() == true) {
            ...
            $payment = array(
                ...
                'bank_slip_no' => $this->input->post('bnkslip_no'),
